...
to plain MLWE can reach it.  The assumption itself, its quantum analogue, and
the simulation details of the one-more hybrids remain open
(Section~\ref{sec:proofneeds}).  Under that assumption the best attacks we
...
common-message strengthening that demotes splicing to duplication, and
blindness.  Section~\ref{sec:efficiency} estimates sizes.
...
Formally, the game to analyze is the dishonest-signer blindness experiment
$\Exp^{\mathrm{blind}}_{\mathcal A}(\lambda)$: $\mathcal A$, playing the
signer, chooses a public key and runs two issuance sessions with the honest
user algorithm $\U$ on inputs of its choice; the experiment hands $\mathcal A$
the two resulting credentials $(\sigma_0,\sigma_1)$ in an order selected by a
hidden bit $b\sample\{0,1\}$, and $\mathcal A$ outputs a guess $b'$.  The
conjecture is
\[
 \Adv^{\mathrm{blind}}_{\mathcal A}(\lambda)
 =\bigl|\Pr[b'=b]-\tfrac12\bigr|=\mathrm{negl}(\lambda).
\]
The intuitive support is that a presented credential is statistically far from
...
the user, and the tags are pseudorandom.  Making this precise is real work
that we do not do here: it needs a sound
...
leaf hashes of selected branches visible to the signer: the experiment
$\Exp^{\mathrm{blind}}$ of the blindness subsection, carried through.
