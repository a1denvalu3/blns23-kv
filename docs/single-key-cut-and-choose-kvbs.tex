...
$Q_h\,2^{-\kappa/2}$.  What remains heuristic is the step from uncontrolled
responses to lattice hardness, the one-more accounting, and the quantum
random-oracle extension (Section~\ref{sec:proofneeds}).  Under that remaining
intuition the best attacks we know are the copying-and-combining attacks of
...
Theorem~\ref{thm:fixpoint}).  The proof does not extend to quantum query
access, the reduction from uncontrolled responses to a standard lattice
assumption remains heuristic, and one-more unforgeability is unaccounted
for (Section~\ref{sec:proofneeds}).
...
opening.  No replacement reduction is known.  Security here rests on the
interaction of the challenge hash with the aggregate, which we call
\emph{structured-sum fixpoint hardness}; we prove its challenge-game content
in the classical random-oracle model
(Section~\ref{sec:romproof}), while the step from its conclusion to standard
lattice hardness remains unstudied.
...
game, the splicing attacks and their costs, the
...
(Section~\ref{sec:romproof}), and the parts that remain heuristic.
...
attacker can influence but not aim, and standard intuition applies: key
recovery from uncontrolled samples is the MLWE problem, and the value
$\mathbf s^{T}H_R(\cdot)$ at a fresh, session-bound input behaves like a
pseudorandom function (this is the HMLWE heuristic of BLNS23, modeled on its
Lemma~4.3).  That second step --- from uncontrolled samples to lattice
hardness --- is \emph{not} covered by the random-oracle analysis and remains
heuristic.
...
relaxed targets.  Bounding $|T|$ rigorously is part of what a proof of
key-recovery resistance must do (Section~\ref{sec:proofneeds}); the
target-avoidance lemma below (Lemma~\ref{lem:target}) makes the dependence
explicit.  Second, the argument is parametric: it rests on
...
margin, which is why bounding the dangerous target set is listed as proof
work in Section~\ref{sec:proofneeds}.
...
key-recovery resistance: Theorem~\ref{thm:fixpoint} says the attacker cannot
aim $\mathbf C_{\mathrm{sel}}$, not that
$h=\mathbf s^{T}\mathbf C_{\mathrm{sel}}+e$ with uncontrolled
$\mathbf C_{\mathrm{sel}}$ is safe; that step is the MLWE/HMLWE heuristic
discussed above, and it is where the sizes of $T$ and $T_{\mathrm{rel}}$ must
be justified.  Third, one-more unforgeability: the rectangle relation
combines responses across sessions regardless of how uncontrolled each
session is individually, and the accounting of
Section~\ref{sec:commonmessage} is outside the challenge game entirely.
...
the splice sits exactly there.  What the analysis does not give is a proof
that no \emph{use} of such a relation (the one-more accounting of
Section~\ref{sec:proofneeds}) is possible below it.
...
prerequisite for the one-more accounting that any proof would need
...
Three honest qualifications.  First, the demotion is itself only intuition:
we know of no counting argument that would justify it rigorously here, and
with a single key there is no fixed untouched lane on which such an argument
could anchor.  Second, the binding does nothing against key recovery through
...
which covers the challenge game but not the lattice step.
...
challenge-game content of the fixpoint property.  A full security proof of
the construction would additionally need:
...
avoid extraction entirely.
 \item \textbf{A quantum random-oracle analysis, and target-set bounds.}
...
or equivalent techniques.  Separately, the bounds scale linearly with the
committed target and relation sets: a proof of key-recovery resistance must
bound how many peeled sums (or differences of peeled sums) key material is
actually extractable from, in the presence of the correctness noise.
 \item \textbf{One-more accounting.}  Even with pseudorandom responses, the
rectangle relation means responses combine.  A proof needs the common-message
binding of Section~\ref{sec:commonmessage} (or an equivalent) and a counting
argument that an extra distinct message forces a fresh secret-key evaluation
--- and no fixed untouched lane exists in this construction on which such an
argument could be anchored.
...
size of the ring (Section~\ref{sec:wagner}).  What remains open is everything
past the challenge game: the step from uncontrolled responses to lattice
hardness, the quantum random-oracle extension, and the one-more accounting.
