// Textbook SHA-256 (FIPS 180-4). See sha256.hpp.
...
void sha256(const uint8_t *in, size_t inlen, uint8_t out[32]) {
  ...
  tail[rem++] = 0x80;
  size_t pad_end = (rem <= 56) ? 56 : 120;
  std::memset(tail + rem, 0, pad_end - rem);
  uint64_t bitlen = static_cast<uint64_t>(inlen) * 8;
  for (int i = 0; i < 8; i++)
    tail[pad_end + i] = static_cast<uint8_t>(bitlen >> (56 - 8 * i));
  compress(h, tail);
  if (pad_end == 120)
    compress(h, tail + 64);
