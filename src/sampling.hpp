#pragma once
//
// Samplers over R_Q. Deterministic when driven by a seeded byte source.
// The source type D must expose the Drbg interface (fill/bytes/next_below);
// both Drbg (SHAKE256) and Poseidon2Xof qualify.
...
typename Ring<P>::Poly sample_uniform(const Ring<P> &R, D &drbg);

// Ternary: coefficients uniform in {-1, 0, +1} (same integer in every slot).
template <typename P, typename D>
typename Ring<P>::Poly sample_ternary(const Ring<P> &R, D &drbg);

template <typename P, typename D>
typename Ring<P>::Vec sample_uniform_vec(const Ring<P> &R, D &drbg);

template <typename P, typename D>
typename Ring<P>::Vec sample_ternary_vec(const Ring<P> &R, D &drbg);
